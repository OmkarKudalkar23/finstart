...
import { Upload, AlertCircle, CheckCircle2, X, FileText, Sparkles, Eye, EyeOff } from "lucide-react";
              ...
              <div className="bg-black/40 border border-white/10 rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={handleRetry}
                      className="px-3 py-1.5 bg-white/10 hover:bg-white/20 text-white/70 rounded-lg transition-colors text-xs"
                    >
                      Choose Different
                    </button>
                  </div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <span className="ml-2 text-sm text-white capitalize">{ocrResult.documentType || "Unknown"}</span>
                  </div>
                  ...
                  <div className="space-y-2">
                    {getMistralOCRService().validateDocumentFields(ocrResult.extractedFields || {}).errors.map((msg, idx) => (
                      <div key={`err_${idx}`} className="flex items-center gap-2 text-red-400 text-sm">
                        <AlertCircle className="w-3 h-3" />
                        {msg}
                      </div>
                    ))}
                    {getMistralOCRService().validateDocumentFields(ocrResult.extractedFields || {}).warnings.map((msg, idx) => (
                      <div key={`warn_${idx}`} className="flex items-center gap-2 text-yellow-400 text-sm">
                        <AlertCircle className="w-3 h-3" />
                        {msg}
                      </div>
                    ))}
